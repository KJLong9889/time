...
TSFEATURE_NAMES = [
    "max_kl_shift", ###max_kl_shift（最大KL散度偏移）
    "max_level_shift",# max_level_shift（最大均值偏移）
    "max_var_shift", ####max_var_shift（最大方差偏移）
    "acf_features",
    "arch_stat",
    "crossing_points",
    "entropy",
...
class TimeSeriesFeatureExtractor:
    ...
    def read_data(self, path: str, nrows=None) -> pd.DataFrame:
        ...
        支持以下格式：
        1. 标准时序数据 (item_id, timestamp, value): 自动透视转换为宽格式。
        2. 旧版特殊格式 (包含 'cols' 列): 执行旧版解析逻辑。
        3. 简单数值数据: 仅读取数值列。
        """
        try:
            # 尝试读取文件
        ...
        # ==================================================
        # 1. 优先尝试：标准 TimeSeriesDataFrame 格式适配
        #    自动寻找 item_id 和 timestamp 列，并进行 Pivot
        # ==================================================
        
        # 定义可能的列名别名
        time_aliases = ['timestamp', 'date', 'time', 'datetime']
        id_aliases = ['item_id', 'id', 'series_id', 'item']
        
        # 查找是否存在对应的列
        time_col = next((c for c in time_aliases if c in df.columns), None)
        id_col = next((c for c in id_aliases if c in df.columns), None)
        
        # 如果同时找到了 时间列 和 ID列，说明是长格式数据，需要透视
        if time_col and id_col:
            # 找到数值列（排除掉 时间列 和 ID列 之外的第一列通常是数值）
            val_cols = [c for c in df.columns if c not in [time_col, id_col]]
            ...
            if val_cols:
                ...
                try:
                    ...
                    # 执行透视：行=时间, 列=ID, 值=目标值
                    df_wide = df.pivot(index=time_col, columns=id_col, values=target_col)
                    
                    # 如果有行数限制
                    if nrows is not None:
                        df_wide = df_wide.iloc[:nrows, :]
                    
                    # 某些算法不支持 datetime 索引，重置索引并只保留数值内容（视你的算法需求而定）
                    # 但通常 feature_extract 需要纯数值 DataFrame（列是序列名，索引是时间或只是序号）
                    # 如果你的算法不依赖索引是时间，可以直接返回
                    return df_wide
                    
                except Exception as e:
                    print(f"Pivot failed, falling back to other methods: {e}")
                    # 如果透视失败，继续往下走兜底逻辑

        # ==================================================
        # 2. 兼容旧逻辑：如果数据中包含 'cols' 列
        # ==================================================
        if "cols" in df.columns:
            try:
                # 原始旧代码逻辑
                label_exists = "label" in df["cols"].values
                
                # 确定时间点数量
                if df.columns[0] == "date":
                    n_points = df.iloc[:, 2].value_counts().max()
                else:
                    # 假设第二列是某种标识
                    n_points = df.iloc[:, 1].value_counts().max()

                all_points = df.shape[0]
                n_cols = all_points // n_points
                
                # 提取列名
                cols_name = df["cols"].unique()
                ...
                for i in range(len(cols_name)):
                    # 假设 value 在最后一列 (索引 -1)
                    # 注意：这里假设 cols 列可以用来筛选
                    col_data = df[df["cols"] == cols_name[i]].iloc[:, -1].values
                    # 截断或填充以匹配 n_points (简单处理，直接赋值)
                    # 这里为了安全，只取前 n_points 个，防止长度不一致报错
                    if len(col_data) > n_points:
                        col_data = col_data[:n_points]
                    
                    new_df[cols_name[i]] = pd.Series(col_data)
                
                if nrows is not None:
                    new_df = new_df.iloc[:nrows, :]
                    
                return new_df
                
            except Exception as e:
                print(f"Legacy 'cols' parsing failed: {e}")
                # 旧逻辑失败，继续往下走
        ...
        # 如果过滤后没数据了，或者原本就是空的，就原样返回试试
        if numeric_df.empty:
            return df
            
        if nrows is not None:
            numeric_df = numeric_df.iloc[:nrows, :]
    ...
    def fft_transfer(
        ...
        """  ###########周期性，寻找频谱中振幅最大的频率，将其转换为周期。使用 adjust_period 将计算出的周期强制映射到常见的物理周期（如24小时、7天、1年等）
    ...
    def feature_extract(self, path: str) -> pd.DataFrame:  ###调用了 STL 进行分解
        """
        ...
        index_columns = [
            ...
            "if_season",
            "if_trend",
            "ADF:p-value", ################adfuller (ADF检验) 和 kpss (KPSS检验)
            "KPSS:p-value",
            "stability",
            "skewness",
            "kurt",
...
class TimeSeriesProcessor:
    def __init__(self, output_dir: str = "characteristics"):
        ...
        if self.output_dir is None:
            import tempfile
            self.output_dir = tempfile.mkdtemp()  # 如果没有指定目录，创建一个临时目录
...
if __name__ == "__main__":
    processor = TimeSeriesProcessor(output_dir="/home/admin123/phao/M/fenxi_results")
    file_path = "/home/admin123/phao/M/data/dataset/converted-electricity.csv"  # supports input a single file
